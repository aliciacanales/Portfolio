...
# Read coral data
coral <- readxl::read_excel(here('data', 'shiny_app_data', 'coral_data_244_akd.xls')) %>% 
  mutate(date = ymd(date)) %>% 
  mutate(site = as.integer(site))

# Filter and clean coral data for Pocillopora and Acropora genera
poc_acr <- coral %>%
  filter(genus %in% c('poc', 'acr')) %>% 
  mutate(as.factor(site)) %>% 
  mutate(genus = fct_drop(genus))

# Select relevant columns for new_coral dataset
new_coral <- poc_acr %>% 
  select(site, lat, long, genus) %>% 
  group_by(site)

# Count observations by genus and site
counts <- new_coral %>%
  group_by(genus) %>%
  count(site)
...
# Count observations by genus and site, rename column
counts_na <- coral %>%
  group_by(genus) %>%
  count(site) %>% 
  rename("total_n" = n)

# Count total observations by site
counts_na_ng <- coral %>% 
  count(site) %>% 
  rename("total_n" = n)

# Merge genus counts and total counts by site
gc <- counts %>% 
  select(site, genus, n)

# Calculate percent dead and summarize by genus and site
dead_counts <- coral %>%
...
# Merge dead coral counts and genus counts by site
dc <- dead_counts %>% 
  select(site, genus, average_of_perc_dead)

# Combine genus counts, dead coral counts, and total counts by site
site_class <- full_join(dc, gc)

# Join with total counts by site
...
# Group coral counts by plot and bommie location
plot_counts <- coral %>% 
...
# Replace 'N/A' with 'Undetermined' in bommie_loc column
plot_counts$bommie_loc[plot_counts$bommie_loc == 'N/A'] <- 'Undetermined'

### For the reactive map :

# Prepare lat-lon data for reactive map
lat_lon_cor <- coral %>% 
  select(site, lat, long) %>% 
  merge(counts_na_ng, by = ('site')) %>% 
  unique()

# Count coral observations by genus and site, filter for garden observations
garden_counts <- coral %>% 
...
# Merge genus counts and new_coral data, convert to spatial data frame
comb_coral <- counts %>% 
  merge(new_coral, by = c('site', 'genus')) %>% 
  unique() %>% 
  mutate(site = as.factor(site))

# Read shapefile and filter for French Polynesia
fp <- read_sf(here::here("data","xg569rm6446.shp")) %>%
  filter(hasc_1 == "PF.WI") %>%
  select(name_0, varname_1, geometry)

# Create base map with shapefile data
...
# Summarize percent dead by bommie location
bom_dead <- coral %>% 
...
# Create bar plot for bommie locations
...
# Summarize percent dead by site and bommie location
site_bom <- coral %>% 
...
# Generate HTML table for metadata
meta_kable <- knitr::kable(metadata)  

### Theme:

# Define custom theme for Shiny app
my_theme <- bs_theme(
  bg = 'lightblue',
  fg = 'white', 
  primary = 'white',
